...
# check (and maybe improve) the task() interval to ensure it is 15 seconds

# Add status flags to Modbus RTU register for Dust, Ecom & Logger health
# Finish PDF report layout
# allow user defined passwords?  or 2 level passwords

# Quick notes on operation:
#     The user can define a storage intreval.  The system averages all the LogInt readigs and logs them to a .csv file
#     at the interval set by the user.
#     The grapgh updates every time the 4-20's are read in and can display up to 2 hours of data.

#########################################
#######   DRM SETTABLE OPTIONS    #######
#########################################
# Line numbers below are approximate, they move down as new code is written!!!  Try searching for phrase in quotes instead if not found
# "O2CutOff =" sets the max level at which O2 corrections are applied.  Note - O2 at 20.95 will cause a divide by zero error 
# Set line 163, "Slave_ID =" to 0 if Modbus is not in use
# DATA STORAGE LOCATION - change default file storage location on line 71 - "SavePath = "
# LOGGING INTERVAL - set LogInt on line ~3251 to the required interval for reading in data from the 4-20 inputs in seconds "LogInt = "
# for Crems, set LogInt to 15
# line ~3251 is used to chaneg log inteval for testing, normally set to 60 (i.e. 1 minute) - LogInt = 60 #in seconds
# line ~3897 set to #"root.attributes"('-zoomed', True) to start with screen maximised
# uncomment line 587 to force a default file rebuild "# CreateDefault()"
# set line 157 to "UseRelays =" 1 if additional optional relay board is fitted
# Use of passwords can be set on or off using line 160 "UsePassword =" "N" OR "Y" - Note factory Password is 41813
# line 158, Enter how many 4 channel 4-20mA input boards are installed "FTBoards_Installed =" 1 or 2
# lines 178 to 193 are used to define some cabinet modbus settings that are not user selectable at the moment
# ModxReg = is the number of the register that is read in, for example, if reading in a block of 10 registers, entering a 3 gives the third one of the block
# This might be best changed to an actual register number at some stage!
# ModCorrectx = set a vlue that the incoming results will be divided by (This is used by Ecom when sending data and needs to follow their protocol)
# line 196, set "TCP_Modbus =" 1 if cabinet TCP network to Ecom is in use
# "DataHold =" used to set how long the system holds the last measured value before an alarm is triggered (based on Ecom purge times)
# "FailSafe = " this decides if alarm relatys operate in failsafe or not
...
os.system ("sudo pigpiod") #Launching the GPIO library
time.sleep(1) # delay required to prevent an error
...
Ch7Units = "mg/m3"
Ch8Units = "mg/m3"
# Set Slave_ID to 0 if RTU modbus not in use
Slave_ID = 1
Modbus_Baud = 9600
Modbus_Bits = 8
Modbus_Parity = 0
...
if TCP_Modbus != 0:
    #Setup TCP modbus network with Ecom
    client = ModbusTcpClient(TCP_IP, port = 502)
...
FileName = SavePath + 'NS.drm'
with open(FileName) as snf:
    Serial_no = snf.readline()
...
def About():
    def BlueUp():
        ...
        BlueInfo['text']="Awaiting update file on Bluetooth"
        BlueInfo.update()
        call("sudo hciconfig hci0 piscan", shell=True)
        BlueCount['text'] = "Waiting"
        for i in range(45):
            BlueCount['text'] = BlueCount['text'] + "."
            BlueCount.update()
            time.sleep(1)
        BlueInfo['text']="Bluetooth Timeout"
        BlueInfo.update()
        print("Stopping discoverable")
        call("sudo hciconfig hci0 noscan", shell=True)
        time.sleep(2)
        BlueInfo['text']=""
        BlueCount['text']=""
        # check to see if a new file has arrived
        FilesArray = os.listdir(SavePath)
        for i in range(len(FilesArray)):
            if len(FilesArray[i])> 12:
                FileSegment = FilesArray[i][-12:]
                if FileSegment == "MCP_Plus.pyc":
                    print(FilesArray[i])
                    print("replace file name")
                    print(FileSegment)
                    # delete existing .pyc file
                    os.remove(SavePath+"MCP_Plus.pyc")
                    # rename received file as original name 
                    os.rename(SavePath+FilesArray[i], SavePath+"MCP_Plus.pyc")
                    messagebox.showinfo("INFO", "Updated version received, System must be re-started")
        
    def RestartSys():
        AboutWindow.lower()
        response = messagebox.askokcancel("Please Confirm", "Restart the system - Are you sure?")
        AboutWindow.tkraise()
        if response == TRUE:
    ...
    def CloseAb():
        if BlueInfo['text']!="Awaiting update file on Bluetooth":
    ...
    # create popup "About" window
    AboutWindow = Toplevel(root)
    AboutWindow.title("About")
    
    # add frames
    AbFrame1 = Frame(AboutWindow, bd=2, padx=10, pady=1, relief=FLAT)
    AbFrame1.pack(expand=True, side=TOP, fill=BOTH)
    AbFrame2 = Frame(AboutWindow, bd=2, padx=10, pady=1, relief=FLAT)
    AbFrame2.pack(expand=True, side=TOP, fill=BOTH)
    AbFrame3 = Frame(AboutWindow, bd=2, padx=10, pady=1, relief=FLAT)
    AbFrame3.pack(expand=True, side=TOP, fill=BOTH)

    LogoImage = Label(AbFrame1, image = Ablogo, width = 91, height = 41)
    LogoImage.pack( side = TOP )

    AboutLabel1 = Label(AbFrame2, text="Version 4.20", padx=10, pady=1)
    AboutLabel1.pack(side=TOP)
    
    AboutLabel2 = Label(AbFrame2, text="Serial No. - "+str(Serial_no), padx=10, pady=1)
    AboutLabel2.pack(side=TOP)
    
    AboutLabel3 = Label(AbFrame2, text="Support:"+"\n"+"www.drmtechnic.com", padx=10, pady=1)
    AboutLabel3.pack(side=TOP)
    
    AbDone_Button = Button(AbFrame3, text="Enable Bluetooth Software Update", wraplength = 160, width = 12, padx=10, pady=2, command=BlueUp, bg=ButtonColour)
    AbDone_Button.pack(side=BOTTOM, padx=3)
    
    Restart_Button = Button(AbFrame3, text="Restart System", wraplength = 160, width = 12, padx=10, pady=2, command=RestartSys, bg=ButtonColour)
    Restart_Button.pack(side=BOTTOM, padx=3)
    
    ConfigSys_Button = Button(AbFrame3, text="Configure System", wraplength = 160, width = 12, padx=10, pady=2,command=lambda: CheckPassword(ConfigSys), bg=ButtonColour)
    ConfigSys_Button.pack(side=BOTTOM, padx=3)
    
    
    BlueInfo = Label(AbFrame2, text="", padx=10, pady=1)
    BlueInfo.pack(side=TOP)
    
    BlueCount = Label(AbFrame2, text="", padx=10, pady=1)
    BlueCount.pack(side=TOP)
    
    AbDone_Button = Button(AbFrame3, text="Done", width = 5, padx=10, pady=20, command=CloseAb, bg=ButtonColour)
    AbDone_Button.pack(side=BOTTOM, padx=3)


def ConfigSys():
    # Read in Board configs and update drivers from web
    # Get todays date to check update list
    date2day = datetime.now()
    date2dayS = str(date2day.strftime("%m%d%Y"))
    Serial_Alone = Serial_no[:-1]
    this_drv = date2dayS+str(Serial_Alone)+".drv"
    gauth = GoogleAuth()
    gauth.LocalWebserverAuth()
    drive = GoogleDrive(gauth)
    DRV_File = drive.CreateFile({'title':this_drv})
    DrvIs = GetDrvInfo()
    DRV_File.SetContentString(DrvIs)
    DRV_File.Upload()
    ErrorNow = 0
    DrvIs = GetDrvInfo()
    if DrvIs == "ERROR000000000":
        ErrorNow = 1
    if DrvIs == "0000000000000000":
        ErrorNow = 1
    DrvFileName = DrvIs + ".inf"
    try:
        f = open(DrvFileName,'w')
        f.write("DRM")
        f.close()
    except:
        ErrorNow = 1
    if ErrorNow == 1:
        messagebox.showinfo("ERROR", "Unable to load drivers")
        root.destroy()
    response = messagebox.showinfo("Done", "New System Config Check complete")
...
def ProtCheck():
    ...
    if (dris.get_sdsn(mydris) != MY_SDSN):
        print("Security Dongle protection error, wrong SDSN")
    ...
    if (dris.get_prodcode(mydris) != MY_PRODCODE):
        print("Security Dongle protection error, wrong product code")
...
def GetDrvInfo():
    ...
    try:
        ...
        DrvInfo = "ERROR000000000"
    #messagebox.showinfo("Drv Info", DrvInfo)
    return DrvInfo
...
def CheckConfig():
    ...
    if DrvIs == "0000000000000000":
        ErrorNow = 1
    DrvFileName = SavePath + DrvIs + ".inf"
    
    try:
        # messagebox.showinfo("Opening", DrvFileName)
        f = open(DrvFileName,'r')
        f.close
    except:
        ErrorNow = 1
    if ErrorNow == 1:
        response = messagebox.askokcancel("ERROR", "No driver found, attempt to reload?"+ "\n" +"(note - Requires internet connection)")
        if response == True:
            ConfigSys()
        else:
            root.destroy()
    else:
        #messagebox.showinfo("Message", "Collected driver info for current board")
        return DrvIs
...
def task():
    ...
    #volt1 = testdata
    print("Current Dust is "+str(volt1))
    
    
    # If mA is below 4mA - Auburn may be doing a self test for next 6 mins 10 seconds
    # If mA is low, but timer is not running, start a 7 minute timer
    # if mA goes high before timer reaches 7 mins, keep holding data until 7 mins is up
    # if timer hits 7 minutes, flag an alarm if 4-20ma is still low
    DustAlarm = 0
    if volt1 < (CH120mA / 16) * -2:
        
        # If timer not already running, start the timer (set HoldDustAt to start time)
        if HoldingDust == 0:
            HoldingDust = 1
            # Make a note of the current time (in Minutes)
            now = datetime.now()
            HoldDustHr = int(str(now.strftime("%H")))
            HoldDustMin = int(str(now.strftime("%M")))
            if HoldDustHr == 0:
                HoldDustHr = 24
            HoldDustAt = (HoldDustHr * 60) + HoldDustMin
            print("Holding dust value from now - 4-20mA is low")
            
        elif HoldingDust == 1:
            # if timer is running, look at current time and calc number of minutes it's been running
            now = datetime.now()
            HoldDustHr = int(str(now.strftime("%H")))
            HoldDustMin = int(str(now.strftime("%M")))
            if HoldDustHr == 0:
                HoldDustHr = 24
            HoldNow = (HoldDustHr * 60) + HoldDustMin
        
            SoFar = HoldNow - HoldDustAt
            print("Dust on hold for - "+str(SoFar))
            if SoFar > drop420time-1:
                # messagebox.showinfo("Dust Self Test", "ALARM, exceeded 6 mins")
                DustAlarm = 1
    
    else:
        # 4-20 signal is high enough to be valid
        ...
        if HoldingDust == 1:
            ...
            SoFar = HoldNow - HoldDustAt
            print("Dust on hold for - "+str(SoFar))
            if SoFar < drop420time:
                print("Keep holding - 4-20 is back online but timer is under "+str(drop420time)+" mins")
                #keep holding
            else:
                HoldingDust = 0
        # If within 6 minutes 10 seconds, keep holding.  If greater than 7 minutes alarm
    ...
    if MCPAlarmCode[3:4] == "1":
        ...
        AlarmFlag['bg'] = "red"
    if DustAlarm == 1:
        # Dust system singal below 4mA for > 5 mins
        AlarmCount += 1
        AlarmFlag['text'] += "\n" +"Auburn General Fault"
        AlarmDesc['text'] = "ALARM"
        AlarmFlag['bg'] = "red"    
...
#plt.plot(x2, y2, label='Second Line')
    ...
    # Update Modbus RTU registers with all data:
    # global context
    print("Updating Modbus RTU server with latest data")
    if Ave_Available == 0:
        CH1_Average = 0
        CH2_Average = 0
        CH3_Average = 0
    ...
    builder.add_16bit_int(CH2_Raw)
    builder.add_16bit_int(CH3_Raw)
    builder.add_16bit_int(CH4_Raw)
    builder.add_16bit_int(CH5_Raw)
    builder.add_16bit_int(CH6_Raw)
    builder.add_16bit_int(CH7_Raw)
    builder.add_16bit_int(CH8_Raw)
    builder.add_16bit_int(CH1_Corr)
    builder.add_16bit_int(CH2_Corr)
    builder.add_16bit_int(CH3_Corr)
    builder.add_16bit_int(CH4_Corr)
    builder.add_16bit_int(CH5_Corr)
    builder.add_16bit_int(CH6_Corr)
    builder.add_16bit_int(CH7_Corr)
    builder.add_16bit_int(CH8_Corr)
    builder.add_16bit_int(int(CH1_Average))
    builder.add_16bit_int(int(CH2_Average))
    builder.add_16bit_int(int(CH3_Average))
    builder.add_16bit_int(int(CH4_Average))
    builder.add_16bit_int(int(CH5_Average))
    builder.add_16bit_int(int(CH6_Average))
    builder.add_16bit_int(int(CH7_Average))
    builder.add_16bit_int(int(CH8_Average))
    
    AllData = builder.to_registers()
    context[slave_id].setValues(FunctionCode,Register,AllData)
    # format of line below: register = modbus function code,
    # Function codes are:
    #01 - Read Output Coils (0xxxx)
    #02 - Read Input Coils (1xxxx)
    #03 - Read Holding registers (4xxxx)
    #04 - Read Internal Registers (3xxxx)
    #05 - Write Output Coil (force a coil)
    #06 - Write to a single holding register (4xxxx)
    #15 - write to multiple output coils (0xxxx)
    #16 - write to multiple holding registers (4xxxx)
    #22 - bit writes within a holding register (4xxxx)
    #context[slave_id].setValues(FunctionCode,Register , AllData)
 
# Graph setup
root = Tk()
# UNCOMMENT TO DEFAULT TO MAX SCREEN SIZE
root.attributes('-zoomed', True)
root.title("MCP Plus - Combustion Plant Data Processing System")

# create Tkinter drawing area
...
#a.plot(x2, y2, label='Second Line')

# HERE IS WHERE THE GUI LAYOUT IS DEFINED
# There are permanently visible frames
...
#ButtonH = Button(ControlsFrame, text="Test Button", wraplength=70, padx=5, pady=8, command=Test_Button, bg=ButtonColour)
#ButtonH.pack( side = LEFT, padx=4 )
...
#line1.autoscale(a, enable=True, axis='y', tight=None)
f.canvas.draw()
...
#print(TheCheck)
if TheCheck != 0:
    # security dongle not found, exit prpgram
    messagebox.showinfo("ERROR", "No software security dongle found, program will exit")
    root.destroy()
CheckConfig()
#if KillMeNow == 1:
#    os.system('python3 /home/pi/MCP_Plus/MCP_Plus.pyc')
#    #time.sleep(10)
#    sys.exit()
